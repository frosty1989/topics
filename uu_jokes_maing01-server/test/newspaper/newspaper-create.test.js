...
test("HDS", async () => {
  ...
  let newspaperName = "(Mg,Fe2+)2(Mg,Fe2+)5Si8O2(OH)2";
  let response = await TestHelper.executePostCommand(CATEGORY_CREATE, { name: newspaperName });
  expect(response.status).toEqual(200);
  let dtoOut = response;
  expect(dtoOut.name).toEqual(newspaperName);
...
test("A1 - jokesInstance does nto exist", async () => {
  ...
  try {
    ...
    expect(e.code).toEqual("uu-jokes-main/newspaper/create/jokesInstanceDoesNotExist");
...
test("A2 - jokes instance is closed", async () => {
  ...
  try {
    ...
    expect(e.code).toEqual("uu-jokes-main/newspaper/create/jokesInstanceNotInProperState");
...
test("A3 - unsupported keys in dtoIn", async () => {
  ...
  expect(response.status).toEqual(200);
  let warning = response.uuAppErrorMap["uu-jokes-main/newspaper/create/unsupportedKeys"];
...
test("A4 - dtoIn is not valid", async () => {
  ...
  try {
    ...
    expect(e.code).toEqual("uu-jokes-main/newspaper/create/invalidDtoIn");
    expect(e.message).toEqual("DtoIn is not valid.");
  }
});

test("A5 - newspaper with such name already exists", async () => {
  expect.assertions(3);
  await TestHelper.executePostCommand(JOKES_INSTANCE_INIT, { uuAppProfileAuthorities: ".", state: "active" });
  await TestHelper.login("Authority");
  let name = "...";
  ...
  try {
    ...
    expect(e.code).toEqual("uu-jokes-main/newspaper/create/newspaperNameNotUnique");
    expect(e.message).toEqual("Newspaper name is not unique in awid.");
    expect(e.paramMap.newspaperName).toEqual(name);
  }
});

test("A6 - creating newspaper fails", async () => {
  mockDaoFactory();
  ...
  NewspaperAbl.dao.create = () => {
    throw new ObjectStoreError("it fails.");
  };

  try {
    await NewspaperAbl.create("awid", { name: "..." });
  } catch (e) {
    expect(e.code).toEqual("uu-jokes-main/newspaper/create/newspaperDaoCreateFailed");
    expect(e.message).toEqual("Create newspaper by newspaper DAO create failed.");
  }
});
